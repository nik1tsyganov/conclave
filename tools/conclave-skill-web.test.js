...
const { indexSkillWeb, loadSourceContract } = require('./conclave-skill-web.js');
...
function fixture(t) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'conclave-skill-web-'));
  ...
  return {
    ...
    env: {
      CONCLAVE_VAULT_ROOT: vaultRoot,
      CONCLAVE_FIELD_LIBRARY_ROOT: fieldRoot,
      CONCLAVE_VAULT_SKILLS_ROOT: vaultSkillsRoot,
    },
  };
}
...
test('index catalogs field-library and vault-skills without copying them onto seats', t => {
  ...
  assert.ok(fs.existsSync(path.join(f.vaultRoot, 'projects', 'conclave', 'skill-web.md')));
});
