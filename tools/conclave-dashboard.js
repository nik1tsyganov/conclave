...
const STATES = new Set(['RUNNING', 'PASS', 'FAIL', 'AWAITING_ATTESTATION', 'NOT_RUN', 'RETRYABLE']);
...
function createSnapshot(runDir, { nowMs = Date.now() } = {}) {
  ...
  function observe(entry, dispatch, relative, evidenceRelative, number) {
    ...
    if (allowedEvidence && identity) {
      ...
      if (['PASS', 'AWAITING_ATTESTATION'].includes(attempt.status) && (!processResult || processResult.ok !== true || processResult.exitCode !== 0 || !exitConfirmed)) { attempt.status = 'UNKNOWN'; attempt.issue = 'INCOMPLETE_CHILD_EXIT'; }
      if (processResult) {
        event(dispatch, number, 'child_close', exitAt, exitConfirmed ? (processResult.ok === true && processResult.exitCode === 0 ? 'COMPLETED' : 'FAIL') : 'UNKNOWN', exitConfirmed ? 'Child close recorded; this is not dispatch acceptance' : 'Child exit remains unconfirmed', `${evidenceRelative}/process-result.json`);
        attempt.lastEventAt = newest([attempt.lastEventAt, exitAt]);
      }
  ...
  for (const entry of plan.dispatches) {
    ...
    const original = observe(entry, dispatch, `.conclave-dispatches/${key}.json`, `out/${entry.dispatchId}`, 0);
    ...
    for (let number = 1; number <= 2; number += 1) {
      const recovery = `.conclave-recoveries/${key}${number === 2 ? '.2' : ''}`;
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 2) {
    ...
    if (!['--run-dir', '--port'].includes(flag) || seen.has(flag) || !value || value.startsWith('--')) fail('Usage: node tools/conclave-dashboard.js --run-dir <directory> [--port <0-65535>]');
    seen.add(flag);
...
async function main(argv = process.argv.slice(2)) {
  try {
    ...
    process.stdout.write(`CONCLAVE read-only dashboard: ${dashboard.url}\n`);
    return dashboard;
