...
function analyzeRows(rows) {
  ...
  const findings = [];
  if (!rows.length) findings.push({ id: 'capture:zero-rows', severity: 'attention', detail: 'vault telemetry is empty; later CONCLAVE analysis has nothing to score' });
...
function renderAnalysisMarkdown(report) {
  const lines = [
    '# CONCLAVE vault telemetry analysis',
    '',
  ...
  lines.push('', 'This file is the durable CONCLAVE analysis snapshot. Raw JSONL stays gitignored.');
  return `${lines.join('\n')}\n`;
}
...
function main(argv = process.argv.slice(2), io = process) {
  try {
    if (argv.length) throw vaultError('Usage: conclave-vault-analyze.js');
