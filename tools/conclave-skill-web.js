...
const { ensureVaultHome, envOf, looksSecret, requireVaultRoot, vaultError } = require('./conclave-vault.js');
...
function resolveSources(options = {}) {
  ...
  const conclave = resolveRuntimePaths({ root: options.runtimeRoot });
  const vaultRoot = options.vaultRoot || (env.CONCLAVE_VAULT_ROOT ? requireVaultRoot(options) : null);
  return {
    contract,
    conclave,
    vaultRoot,
    sources: [
      { id: 'conclave-seats', root: conclave.seatSkillsRoot, allowForbidden: false },
      { id: 'ai-ops-methods', root: vaultRoot ? path.join(vaultRoot, 'skills') : null, allowForbidden: false },
      { id: 'field-library', root: resolveOptionalDir(options.fieldLibraryRoot || env.CONCLAVE_FIELD_LIBRARY_ROOT, ['INDEX.md', 'modules']), allowForbidden: false, skillDir: 'modules' },
      { id: 'vault-skills', root: resolveOptionalDir(options.vaultSkillsRoot || env.CONCLAVE_VAULT_SKILLS_ROOT, ['README.md', 'skills/vault-ingest/SKILL.md']), allowForbidden: false, skillDir: 'skills' },
      { id: 'host-store', root: resolveOptionalDir(options.hostSkillsRoot || env.CONCLAVE_HOST_SKILLS_ROOT, []), allowForbidden: true, scanBodies: false },
    ],
  };
}
...
function auditWeb(scanned, contract) {
  ...
  const leaf = scanned.filter((item) => item.sourceId === 'conclave-seats' && item.kind === 'skill').map((item) => item.name);
  for (const skill of scanned.filter((item) => ['field-library', 'vault-skills', 'ai-ops-methods'].includes(item.sourceId) && item.kind === 'skill')) {
    if (leaf.includes(skill.name) && !relatedSources(contract, skill.name).includes('conclave-seats')) {
      findings.push({ id: 'field-on-leaf', severity: 'fail', detail: `${skill.sourceId}/${skill.name} collides with a CONCLAVE leaf card` });
    }
  }
  return findings;
}
...
function renderSkillWebMarkdown(report) {
  ...
  lines.push('', 'Field modules and Obsidian ingest methods stay in their own git repos. This catalog is the link CONCLAVE and the vault read.');
  return `${lines.join('\n')}\n`;
}

function main(argv = process.argv.slice(2), io = process) {
  try {
    if (argv.length) throw vaultError('Usage: conclave-skill-web.js');
