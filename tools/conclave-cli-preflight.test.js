...
const { check, main } = require('./conclave-cli-preflight.js');
...
function fixture(t) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'conclave-preflight-'));
  t.after(() => {
  ...
  const profiles = { arbiterSkills: ['conclave-mode'], forbiddenSeatSkills: ['conclave-mode'],
  ...
  const env = {};
  for (const name of ['CONCLAVE_CODEX_BIN', 'CONCLAVE_AGY_BIN', 'CONCLAVE_CLAUDE_BIN']) {
...
test('explicit environment rules root is supported', t => {
  const f = fixture(t);
  f.env.CONCLAVE_RULES_ROOT = f.rulesRoot;
  delete f.rulesRoot;
...
test('forbidden and unsafe profile skills fail preflight', t => {
  const f = fixture(t);
  for (const name of ['conclave-mode', '../testing']) {
