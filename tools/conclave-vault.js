...
function assertVaultRoot(root) {
  if (!fs.existsSync(root) || !fs.lstatSync(root).isDirectory()) {
    throw vaultError('CONCLAVE_VAULT_ROOT is not a directory; set it to the ai-ops-vault checkout');
  }
  for (const relative of VAULT_MARKERS) {
    const file = path.join(root, ...relative.split('/'));
    if (!fs.existsSync(file) || !fs.lstatSync(file).isFile()) {
      throw vaultError(`CONCLAVE_VAULT_ROOT is not an ai-ops-vault: missing ${relative}`);
    }
...
function resolveVaultRoot(options = {}) {
  const raw = options.vaultRoot || envOf(options).CONCLAVE_VAULT_ROOT;
  if (!raw) return null;
...
function requireVaultRoot(options = {}) {
  ...
  if (!root) throw vaultError('set CONCLAVE_VAULT_ROOT to the ai-ops-vault checkout');
  return root;
}

function vaultLayout(root) {
  const home = path.join(root, 'projects', 'conclave');
  return {
    root,
    home,
