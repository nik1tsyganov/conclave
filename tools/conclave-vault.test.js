...
const { analyzeRows, analyzeVaultTelemetry } = require('./conclave-vault-analyze.js');
const { linkRunToVault } = require('./conclave-vault-link.js');
const { looksSecret, resolveVaultRoot, vaultLayout } = require('./conclave-vault.js');
const { pullInbox, pushSkillsToVault, syncStatus } = require('./conclave-vault-sync.js');
...
function fixture(t) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'conclave-vault-'));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const vaultRoot = path.join(root, 'vault');
  const runtimeRoot = path.join(root, 'runtime');
  put(path.join(vaultRoot, 'Wiki', 'Index.md'), '# Wiki\n');
  put(path.join(vaultRoot, 'HOW-TO-ADD-DATA.md'), '# how\n');
  const references = path.join(runtimeRoot, 'skills', 'conclave-cli', 'references');
  ...
  return { root, vaultRoot, runtimeRoot, env: { CONCLAVE_VAULT_ROOT: vaultRoot } };
}
...
test('looksSecret catches key-shaped text and resolveVaultRoot stays explicit', t => {
  ...
  assert.throws(() => resolveVaultRoot({ env: { CONCLAVE_VAULT_ROOT: f.root } }), /not an ai-ops-vault/);
});

test('tmp run dirs never auto-link even when CONCLAVE_VAULT_ROOT is set', () => {
  assert.equal(shouldLinkVault(path.join(os.tmpdir(), 'conclave-run'), { CONCLAVE_VAULT_ROOT: '/Users/test/src/ai-ops-vault' }), false);
  assert.equal(shouldLinkVault(path.join(path.parse(os.tmpdir()).root, 'conclave-runs', 'run-001'), { CONCLAVE_VAULT_ROOT: '/Users/test/src/ai-ops-vault' }), true);
  assert.equal(shouldLinkVault(path.join(path.parse(os.tmpdir()).root, 'conclave-runs', 'run-001'), {}), false);
  assert.equal(shouldLinkVault(path.join(path.parse(os.tmpdir()).root, 'conclave-runs', 'run-001'), { CONCLAVE_VAULT_ROOT: '/Users/test/src/ai-ops-vault', CONCLAVE_VAULT_LINK: '0' }), false);
});
...
test('skill push, inbox pull, and status drift', t => {
  ...
  put(path.join(f.vaultRoot, 'projects', 'conclave', 'seat-skills-inbox', 'testing', 'SKILL.md'), '# testing from vault\n');
  ...
  put(path.join(f.runtimeRoot, 'seat-skills', 'seat-openai', 'SKILL.md'), '# changed in CONCLAVE\n');
