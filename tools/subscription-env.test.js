...
const allowed = {
  Path: 'synthetic-path', SystemRoot: 'C:\\Windows', USERPROFILE: 'C:\\synthetic-home',
  CODEX_HOME: 'C:\\synthetic-codex', CLAUDE_CODE_OAUTH_TOKEN: 'synthetic-oauth', MAGI_DEV_ROOT: 'C:\\src',
...
for (const vendor of ['openai', 'anthropic', 'google']) {
  for (const role of ['implement', 'verify']) {
    test(`${vendor} ${role} launch receives sanitized subscription environment`, t => {
      ...
      const source = {
        ...allAliases, ...allowed,
        MAGI_CODEX_BIN: path.join(directory, 'codex.exe'),
        MAGI_CLAUDE_BIN: path.join(directory, 'claude.exe'),
        MAGI_AGY_BIN: path.join(directory, 'agy.exe'),
      };
      const launch = buildLaunch({
        vendor, role, cwd: 'C:\\src\\synthetic-work', briefPath,
