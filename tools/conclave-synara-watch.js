...
const JOIN_KIND = 'conclave-dispatch-join';
...
function walkTransactions(root, depth = 0, found = []) {
  ...
  for (const entry of entries) {
    ...
    const full = path.join(root, entry.name);
    if (entry.isDirectory()) {
      if (entry.name === '.conclave-dispatches') {
        let files;
...
function inspectHooks(file) {
  if (!fs.existsSync(file) || !fs.lstatSync(file).isFile()) {
    return { kind: 'hooks-missing', path: file, note: 'synara-capture hooks.json is missing; CONCLAVE Google isolation cannot be confirmed.' };
  }
  const text = fs.readFileSync(file, 'utf8');
  if (/"decision"\s*:\s*"ask"/.test(text) || /\\"decision\\":\\"ask\\"/.test(text)) {
    return { kind: 'hooks-ask', path: file, note: 'synara-capture PreToolUse emits ask; CONCLAVE Google instruction reads will fail.' };
  }
  return null;
}
...
function writeJoinManifest({ runDir, dispatchIds, out, parentPid }) {
  ...
  const manifest = {
    ...
    note: 'Joins CONCLAVE dispatch-run transactions only. synara_wait_for_threads does not join these PIDs. Never rewrite a receipt to PASS.',
  };
  ...
  if (manifest.parentPid && !pidAlive(manifest.parentPid)) {
    ...
    if (rows.some((row) => row.status === 'RUNNING')) {
      findings.push({
        ...
        note: 'Join-manifest parent PID is dead while a CONCLAVE transaction is still RUNNING. Notify only; never rewrite the receipt to PASS.',
      });
    }
  }
  ...
  while (true) {
    ...
    if (!waiting.length && !missing.length) {
      return {
        ...
        settled,
        outcomes,
        note: 'Joined CONCLAVE dispatch-run transactions only. Never rewrite a receipt to PASS.',
      };
    }
...
function watch(opts) {
  ...
  for (const root of opts.runRoots) {
    if (!fs.existsSync(root)) {
      findings.push({ kind: 'run-root-missing', path: root, note: 'CONCLAVE run root does not exist.' });
      continue;
    }
...
function parseArgs(argv) {
  ...
  if (opts.mode === 'scan' && !opts.runRoots.length && !opts.hooks.length) {
    throw new Error('Usage: conclave-synara-watch --run-roots <dir> [--hooks <hooks.json>...] | --wait --run-dir <sealed-run> [--dispatch-id <id>...] | --record-join --run-dir <sealed-run> [--dispatch-id <id>...]');
  }
...
function main(argv = process.argv.slice(2)) {
  try {
    ...
    process.stderr.write(`CONCLAVE_SYNARA_WATCH_FAIL: ${error.message}\n`);
    return 2;
  }
}
