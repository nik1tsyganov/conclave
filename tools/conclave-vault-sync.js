...
const { indexSkillWeb } = require('./conclave-skill-web.js');
...
function conclaveSkillRoots(options = {}) {
...
function compareTrees(leftRoot, rightRoot) {
  ...
  for (const name of new Set([...left, ...right])) {
    if (!left.includes(name)) { drift.push({ skill: name, detail: 'missing from CONCLAVE seat-skills' }); continue; }
...
function syncStatus(options = {}) {
  ...
  const conclave = conclaveSkillRoots(options);
  const skills = compareTrees(conclave.seatSkills, layout.seatSkills);
  const inbox = listSkillNames(layout.inbox).map(assertSkillName);
  const profileDrift = fs.existsSync(layout.seatProfiles) && fs.existsSync(conclave.seatProfiles)
    ? digest(layout.seatProfiles) !== digest(conclave.seatProfiles)
    : !fs.existsSync(layout.seatProfiles);
  return {
    ok: skills.drift.length === 0 && !profileDrift,
    conclaveSkills: skills.left,
...
function pushSkillsToVault(options = {}) {
  const layout = ensureVaultHome(requireVaultRoot(options));
  const conclave = conclaveSkillRoots(options);
  const names = listSkillNames(conclave.seatSkills).map((name) => skillTree(conclave.seatSkills, name).name);
  replaceTree(layout.seatSkills, names, conclave.seatSkills);
  fs.copyFileSync(conclave.seatProfiles, layout.seatProfiles);
...
function pullSkillsFromVault(options = {}) {
  ...
  replaceTree(conclave.seatSkills, names, layout.seatSkills);
...
function pullInbox(options = {}) {
  ...
  for (const name of listSkillNames(layout.inbox)) {
    ...
    const dest = path.join(conclave.seatSkills, name);
...
function main(argv = process.argv.slice(2), io = process) {
  try {
    ...
    if (argv.length !== 1 || !['--status', '--push', '--pull', '--pull-inbox', '--index'].includes(command)) {
      throw vaultError('Usage: conclave-vault-sync.js --status|--push|--pull|--pull-inbox|--index');
    }
