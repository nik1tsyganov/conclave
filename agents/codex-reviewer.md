...
macOS wrapper (rewritten 2026-09-16; the Cursor Task host mode is legacy, the CONCLAVE CLI runtime in `~/src/conclave` is the product). Reviews a change adversarially by driving the local codex CLI as an independent vendor: read-only; typed findings with file:line; exactly one final POSITION line.
