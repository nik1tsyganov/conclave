...
tools: Bash
model: haiku
skills: codex-bridge, mix-mode, conclave-mode, code-minimalism, check-compiler-errors, deslop, verification-before-completion
---

# codex-implementer

macOS wrapper (rewritten 2026-09-16; the Cursor Task host mode is legacy, the CONCLAVE CLI runtime in `~/src/conclave` is the product). Implements a scoped change by driving the local codex CLI as an independent vendor: write-capable inside the declared write scope only, with a mandatory post-run write audit.
