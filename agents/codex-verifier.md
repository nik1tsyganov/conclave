...
`~/.local/bin/codex` (override with `$CONCLAVE_CODEX_BIN`). Never install, update or fall back to another vendor. Run `source ~/.config/conclave/env.sh` first.
...
1. Read the brief file named in the pointer, in full, and the evidence directory it names (lead-captured test output and diff). Claude and Gemini checking seats read evidence instead of running commands; Codex may run the test command read-only.
2. Run the vendor once, read-only. Each claim gets CONFIRMED, REFUTED or UNMEASURED with the evidence line that decides it.
3. Relay the vendor's answer verbatim; end with exactly one `POSITION: APPROVE|REJECT|ABSTAIN` line.
