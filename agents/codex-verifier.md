...
macOS wrapper (rewritten 2026-09-16; the Cursor Task host mode is legacy, the MAGI CLI runtime in `~/src/magi` is the product). Verifies another agent's work claims by driving the local codex CLI as an independent vendor: read-only; fresh eyes, decorrelated errors.
