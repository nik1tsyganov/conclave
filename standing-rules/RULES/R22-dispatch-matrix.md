# R22 — Bind every launch to executable policy
...
Launch-failure retry (added 2026-09-16, adopted from Droppy Code Hydra): a dispatch whose child never got going may be re-dispatched under the same dispatch ID at most twice more, only when the runtime's launch-failure classifier matched the failure (vendor process exited before any tool call with a recognised never-started signature: safeguard refusal at launch, network reconnect loop, authentication missing, spawn error) and no product, evidence or instruction file changed. Each attempt keeps its evidence directory (`out/<dispatchId>.attempt<n>`); exactly one telemetry row is written at the terminal outcome. Any other failure still requires a new dispatch ID and a new sealed plan.
