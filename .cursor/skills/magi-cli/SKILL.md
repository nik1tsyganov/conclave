...
- Every Magi CLI seat brief MUST include the RULES block from `references/brief-rules-block.md` and MUST name `magi-mode`, `magi-dispatch`, and `engineering-orchestrator` SCOPE. Name `mix-mode` on Codex (recommended on all). Name the vendor bridge when dispatching that vendor. Pack: `BRIEF.md`, `VENDOR.md` (`casper_via=agy`), `RULES/INDEX.md` + R01..R20. Fail closed with `node C:\src\magi\tools\cli-brief-rules-check.js --brief <file>` (`cli-smoke.js` runs the same check).
