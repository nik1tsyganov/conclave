...
- Every Magi CLI seat brief MUST include the RULES block from `references/brief-rules-block.md`. Fail closed with `node C:\src\magi\tools\cli-brief-rules-check.js --brief <file>` (`cli-smoke.js` runs the same check).
