...
7. Run `run-finalize.js --run-dir <run-dir>` and `panel-tally.js --run-dir <run-dir> --unit-id <unit>`. Finalize links vault telemetry when `MAGI_VAULT_ROOT` is set. After MAGI skill edits, `magi-vault-sync.js --push`. New vault skills come from `--pull-inbox`.
