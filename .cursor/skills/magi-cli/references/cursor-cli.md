...
- Every Magi CLI seat brief MUST include the RULES block from `.cursor/skills/magi/references/brief-rules-block.md` (`magi-mode`, `magi-dispatch`, `casper_via=agy`, and `RULES/INDEX.md` or `magi-cli-rules` or `STANDING.md`). Always name `engineering-orchestrator` SCOPE. Name `mix-mode` on Codex (recommended on all). Name `codex-bridge` / `gemini-bridge` / `claude-bridge` when dispatching that vendor. Pack: `BRIEF.md`, `VENDOR.md` (`casper_via=agy`), `RULES/INDEX.md` + R01..R20. Fail closed with `node C:\src\magi\tools\cli-brief-rules-check.js --brief <file>` before dispatch. Do not treat PATH gemini as Casper.
...
- When `C:\src\ai-ops-vault\projects\magi-cli-rules` is available on the host, the arbiter SHOULD pass that directory as `extraDirs` on Gemini/`agy` launches (`cli-gemini.js` already accepts `extraDirs`). Absence of the vault dir is not a launch error. Do not pass the vault (or `C:\Users`) as Claude `--add-dir`. Claude `--add-dir` stays cwd + briefDir only.
