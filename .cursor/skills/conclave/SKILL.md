---
name: conclave
description: CONCLAVE in Cursor. Use when the user says conclave or /conclave. Grok routes Claude+Codex+Gemini. Not CONCLAVE.
---
> **Legacy host mode (2026-09-16):** `/conclave` Cursor Task mode predates the macOS runtime. The product is `/conclave-cli` (`.cursor/skills/conclave-cli`, `commands/conclave-cli.md`). Cursor Task electors are not CONCLAVE seats.
...
CONCLAVE is not CONCLAVE. Do not run CONCLAVE's `session-whoami.js` or dispatch `camerlengo-8`.

`/conclave` selects native Cursor Tasks with `hostMode: cursor`. `/conclave-cli` selects native vendor CLIs with `hostMode: cursor-cli`. Run `conclave-whoami` with the selected mode before activation. It checks the declared route, not the actual host model.

## Cursor CLI only

For `cursor-cli`, use the [CONCLAVE CLI skill](../conclave-cli/SKILL.md), [CLI run procedure](references/cursor-cli.md), and [brief rules block](references/brief-rules-block.md). The native workflow below does not apply to CLI runs or their leaves.
...
1. Set `CONCLAVE_RULES_ROOT` to the external STANDING v2 / R01-R22 pack and `CONCLAVE_VAULT_ROOT` to the ai-ops-vault checkout, then run the CLI preflight. Use `claude auth status` for the current Claude login check.
...
Run these tools from the resolved CONCLAVE source checkout or installed runtime directory. The complete field contracts and commands are in the linked CLI run procedure. A standalone matrix check, vendor command, or smoke result does not seal or activate a production run.
...
7. `$HOME\.claude\skills\conclave-mode\SKILL.md`
8. `$HOME\.claude\skills\conclave-dispatch\SKILL.md`
...
- `/conclave` uses hostMode `cursor`: Grok arbiter dispatches native Cursor Task
...
- Before each Cursor elector Task, and after a Task fails with usage or quota
  language, run `node $HOME/src/conclave\tools\host-resolver.js`. If it trips, send
...
Permute the vendors across implement units; Advocatus must not be idle. Review and
...
Project slice lists do NOT assign vendors. Ignore any `implement`/`verify` vendor column in a project `SLICES.md` or plan doc: vendors come from the CONCLAVE class table plus the live floor at dispatch, and a project plan never beats the class table. The only valid owner override is `vendorOverride` on that run's `graph.json` node, recorded there — not in a doc. Canonical policy: `mix-mode` → `distributionFloor.projectSlicePolicy`. Pointer only; do not restate or fork it here.

Each implementer must paste WRITE AUDIT (`git diff --stat` + `git status --porcelain`).

After implement rows, write `conclave-dispatch-log.jsonl` `{vendor, role:"implement"}` (gitignored) and run `node $HOME/src/conclave\tools\activation-check.js <log path>`. Exit 1 = FAILED activation. Do not use `dispatch-log.pass.jsonl` as a live log.
...
- Do not run `conclave-battery.js` or `mix-run.js` under plain node.
- Parse check only: `node $HOME\.claude\workflows\checks\conclave-workflow-cli.mjs`

### Native packs (efficiency, not routers)

Cursor Task seats may use these local Cursor plugin packs as efficiency skills; they do not replace CONCLAVE dispatch:
