---
name: conclave-cli
description: Plan-bound CONCLAVE CLI mode. The Jev decision engine is the arbiter and the hosting session (Cursor, Synara or Claude Code) runs the tools; OpenAI, Anthropic and Google seats use native CLIs. Not CONCLAVE.
---
...
1. Set `CONCLAVE_RULES_ROOT` to the external STANDING v2 / R01–R22 pack. Set `CONCLAVE_VAULT_ROOT` to the ai-ops-vault checkout. Run `tools/conclave-cli-preflight.js`.
...
4. Finalize the briefs, then write the complete plan with unique dispatch IDs, immutable brief hashes, absolute worktrees, relative scopes, author provenance, and any escalation reason. Do not brief seats to inspect CONCLAVE CLI runtime, plugin, vendor-bridge, or dispatcher source.
5. Run `plan-seal.js --plan <draft.json> --run-dir <new-run-dir> --availability <availability.json>`. A `hostMode: synara` plan also requires `--synara-catalog` from `synara-catalog.js --import`. Optional `--skill-source-root` binds skill bytes into seal schemaVersion 2.
...
7. Run `run-finalize.js --run-dir <run-dir>` and `panel-tally.js --run-dir <run-dir> --unit-id <unit>`. Finalize links vault telemetry when `CONCLAVE_VAULT_ROOT` is set. After CONCLAVE skill edits, `conclave-vault-sync.js --push`. New vault skills come from `--pull-inbox`.
