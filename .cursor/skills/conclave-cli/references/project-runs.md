# CONCLAVE CLI: real-project handoff for Cursor
...
2. Reload Cursor after a CONCLAVE runtime update. The host declaration checker does not attest the actual picker model.
3. Paste the prompt below. Replace the two bracketed fields. If `/conclave-cli` is unavailable, attach the installed `commands/conclave-cli.md` and this file to the same prompt.

```text
/conclave-cli

Read and follow the installed CONCLAVE CLI references/project-runs.md handoff.
...
Set `CONCLAVE_FIELD_LIBRARY_ROOT` and `CONCLAVE_VAULT_SKILLS_ROOT` and run `conclave-vault-sync.js --index` after those repos change. The host store is not a seat source. Do not merge field-library or vault-skills into CONCLAVE.
...
```text
C:/src/conclave-project-runs/<project>/<attempt-id>/
...
C:/src/conclave-project-issues/<project>-<attempt-id>/
...
```bash
node tools/run-finalize.js --run-dir $conclaveRun
node tools/conclave-vault-analyze.js
node tools/activation-check.js --run-dir $conclaveRun
node tools/panel-tally.js --run-dir $conclaveRun --unit-id $conclaveUnit
```
...
```bash
node tools/project-run-report.js --run-dir $conclaveRun --project-root $conclaveProject --output-dir $conclaveIssueDir --phase finalize
```

After failure, include the captured failing command output:

```bash
node tools/project-run-report.js --run-dir $conclaveRun --project-root $conclaveProject --output-dir $conclaveIssueDir --phase dispatch --error-file $conclaveFailureLog
```

Use the actual failure phase: `preflight`, `probe`, `plan`, `dispatch`, `verify`, `review`, `finalize`, `activation`, or `project`. Set `$conclaveProject` to the actual product directory. Before a sealed run exists, pass the existing attempt directory as `--run-dir`. Supply `--project-root` when a seal is missing or damaged; unverified plan paths cannot establish the product boundary. Missing or broken seals are recorded as UNVERIFIED. For an activation or project-level failure after otherwise successful dispatches, `--error-file` preserves that failure in the report.
...
When the host is Synara, use `hostMode: synara` and snapshot `synara_capabilities` with `synara-catalog.js` before sealing. Map CONCLAVE vendor names to Synara providers only for catalog checks (`openai`/`codex`, `anthropic`/`claudeAgent`, `google`/`antigravity`). Launch remains `dispatch-run.js`.

Use Synara worktrees as an implement `cwd` when the worktree is already inside CONCLAVE allowed roots. Bind that path with `host-helper-worktree.js` before sealing. Use `browser_*` only as a host helper after CONCLAVE implement; stage those files with `host-helper-evidence.js` and list the destination on verify/review `evidenceReadDirs`. Never tally a Synara-thread helper as a CONCLAVE `POSITION`. `synara_wait_for_threads` does not join `dispatch-run` PIDs. Record a `join-manifest.json` with `conclave-synara-watch.js --record-join`, then `--wait` on the sealed run directory. The same watch scan reports leftover `RUNNING` children and synara-capture `ask` revert without rewriting receipts.

Do not create CONCLAVE seats as Synara threads. Do not substitute Cursor Task elector slugs for seats.

## Scope of this handoff

Start fresh attempts after this update. Implementation-based checker receipts now bind `conclave-unit-sequence-v1` prerequisite evidence. Older checker receipts lack that contract and will fail the updated validator. Preserve those historical runs with their original runtime; do not rewrite their receipts or present them as new acceptance.
