...
- Every Magi CLI seat brief MUST include the RULES block from `.cursor/skills/magi/references/brief-rules-block.md` (vault `C:\src\ai-ops-vault\projects\magi-cli-rules\STANDING.md` or `magi-cli-rules`, `magi-mode`, `magi-dispatch`, `casper_via=agy` or `VENDOR.md` plus Casper/`agy` card text). Fail closed with `node C:\src\magi\tools\cli-brief-rules-check.js --brief <file>` before dispatch. Do not treat PATH gemini as Casper.
