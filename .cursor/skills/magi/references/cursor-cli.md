...
- Every Magi CLI seat brief MUST include the RULES block from `.cursor/skills/magi/references/brief-rules-block.md` (vault `C:\src\ai-ops-vault\projects\magi-cli-rules\STANDING.md`, Read before work + first-line echo, `magi-mode` / `magi-dispatch`, Casper=`agy`). Fail closed with `node C:\src\magi\tools\cli-brief-rules-check.js --brief <file>` before dispatch.
