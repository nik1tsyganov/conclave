...
- Every Magi CLI seat brief MUST paste the STANDING RULES block from `.cursor/skills/magi/references/brief-rules-block.md` (fill SCOPE / Vendor / Bridges at dispatch). Fail closed: `magi-mode`, `magi-dispatch`, `mix-mode`, `casper_via=agy` or `Vendor:`, `RULES/INDEX`, and `WRITE AUDIT` or `R07`. Pack: `BRIEF.md`, `VENDOR.md`, `RULES/INDEX.md` + R01..R21. `node C:\src\magi\tools\cli-brief-rules-check.js --brief <file>` before dispatch.
