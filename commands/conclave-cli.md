...
Critical classes require `conclaveConvened: true` and two distinct foreign review/verify vendors on the same unit and worktree. Implementation vendor count scales as `min(3, implementation units)`; the 60% cap starts at two units. Do not invent implementation rows for read-only panels.
...
- `source ~/.config/conclave/env.sh` before any tool; it exports `CONCLAVE_CODEX_PROVIDER=openai` (the cpa-gui proxy is normally down; the adapter also defaults to it on darwin).
